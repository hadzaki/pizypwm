...
class PiZyPwm(threading.Thread):
  def __init__(self, frequency, gpioPin):
     ...
     """
     self.baseTime = 1 / frequency
     self.maxCycle = 100
     self.sliceTime = self.baseTime / self.maxCycle
     self.gpioPin = port.__dict__[gpioPin]
     self.terminated = False
     self.toTerminate = False
     #GPIO.setmode(gpioScheme)#удалил так как схема обозначения ножек на opi нет
  def start(self, dutyCycle):
    ...
    #GPIO.setup(self.gpioPin, GPIO.OUT)#поменять на следующую строку
    gpio.setcfg(self.gpioPin, gpio.OUTPUT) #Configure as output
  ...
  def run(self):
    ...
    while self.toTerminate == False:
        if self.dutyCycle > 0:
          #GPIO.output(self.gpioPin, GPIO.HIGH)заменить на след строку
          gpio.output(self.gpioPin, gpio.HIGH)#ПОТЯНУТЬ К ПИТАНИЮ
          time.sleep(self.dutyCycle * self.sliceTime)
        if self.dutyCycle < self.maxCycle:
          # GPIO.output(self.gpioPin, GPIO.LOW)#заменить на след строку
          gpio.output(self.gpioPin, gpio.LOW)#ПОТЯНУТЬ К GND
  ...
  def stop(self):
    ...
    # GPIO.output(self.gpioPin, GPIO.LOW)#заменить на след строку
    gpio.output(self.gpioPin, gpio.LOW)#ПОТЯНУТЬ К GND
    #GPIO.setup(self.gpioPin, GPIO.IN)ЗАМЕНИТЬ НА СТРОКУ
    #gpio.setcfg(self.gpioPin, gpio.INPUT) #Configure as output
    #gpio.pullup(self.gpioPin, 0) #Clear pullups
    #gpio.pullup(self.gpioPin, gpio.PULLDOWN) #Enable pull-down
